package model_support::model_supportServer;
...
our %async_run_methods = (
        'count_contigs_async' => 'model_support.count_contigs',
);

our %async_check_methods = (
        'count_contigs_check' => 'model_support.count_contigs',
);
...
sub call_method {
    ...
    my $ctx = model_support::model_supportServerContext->new($self->{loggers}->{userlog},
...
{
    # Service model_support requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for model_support but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = model_support::model_supportServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service model_support.");
    }
...
package model_support::model_supportServerContext;

use strict;

=head1 NAME

model_support::model_supportServerContext
...
package model_support::model_supportServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = model_support::model_supportServer->new(
